...
class Checker(TwoPlayerGame):
    ...
    def make_move(self, pos):
        ...
        ------
        """
        #setting the player's current positions to the passed position argument
    ...
    def lose(self):
        ...
        for v in self.white_territory: #loop through white territory values
            if self.board[v] == 'B': #if a 'B' value is in white territory then return true for lost
                return True
                
        for v in self.black_territory: #loop through black territory values
            if self.board[v] == 'W': #if a 'W' value is in white territory then return true for lost
                return True
        return False #no 'B' or 'W' found in enemy territory; not lost
    ...
    def is_over(self):
        ...
        """
        return (self.possible_moves() == []) or self.lose() #returning no more moves or lose if no more moves is false
    ...
    def scoring(self):
        ...
        lose = -100
        """
        if self.lose(): #return -100 if lost; otherwise 0
            return -100 
        else:
            return 0
